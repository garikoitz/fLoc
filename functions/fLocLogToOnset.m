function fLocLogToOnset(floc_dir,floc_log_folder_name)
...
fLocLogToOnset(floc_dir,floc_log_folder_name)
end;
%}
...
for i = 1:length(task_probe_reshape)
    if task_probe_reshape(i) == 1
        final_stimuli{i} = ['OBTask_' final_stimuli{i} ];  % Append ' task' to the existing string
    end
end
...
for run = 1:num_of_run
    ...
    % Apply the offset
    onset_reshape(startIndex:endIndex) = onset_reshape(startIndex:endIndex) + offsets(run);
end
...
onset_table = table(subID_col, runs, final_stimuli, onset_reshape, ...
    'VariableNames', {'SubjectID', 'Run', 'Cond', 'Onset'});

a=mkdir(fullfile(floc_dir,"onset"));
% Write the table to an Excel file
filename = sprintf('%s_%s_onset.xls', subID, sesID);

writetable(onset_table,fullfile(floc_dir,"onset", filename));
end
