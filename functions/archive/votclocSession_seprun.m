classdef votclocSession_seprun
    ...
    methods
        ...
        function session = votclocSession_seprun(name, trigger, stim_set, num_runs, task_num, use_eyelink)
        ...
        function session = load_seqs(session)
            fname = [session.id '_votclocSequence_seprun.mat'];
            fpath = fullfile(session.exp_dir, 'data', session.id, fname);
            % make stimulus sequences if not already defined for session
            if ~exist(fpath, 'file')
                seq = votclocSequence_seprun(session.stim_set, session.num_runs, session.task_num);
                seq = make_runs(seq);
